...
import { AddCommentT, AuthenticatedRequestBody, IUser, IPost, TPaginationResponse, UpdateCommentT, CommentI, LikeT } from "../interfaces";
...
export const getTimelinePostsService = async (req: AuthenticatedRequestBody<IUser>, res: Response, next: NextFunction) => {
  try {
  ...
  try {
    ...
    return res.status(200).send(
      customResponse<typeof data>({
        success: true,
        error: false,
        message: posts.length ? `Successfully found all posts for user by ID ${req?.user?._id}` : `No post found for user by ID ${req?.user?._id}`,
        status: 200,
        data,
      })
    );
...
export const deleteUserPostsService = async (req: AuthenticatedRequestBody<IUser>, res: Response, next: NextFunction) => {
  try {
    const posts = await Post.find({
      author: req?.user?._id || "",
    })
...
export const likePostService = async (req: AuthenticatedRequestBody<IPost>, res: Response, next: NextFunction) => {
  try {
    ...
    const message = isAlreadyLiked ? `Successfully disliked post by ID: ${req.params.postId}` : `Successfully liked post by ID: ${req.params.postId}`;
    return res.status(200).send(
      customResponse<typeof data>({
        success: true,
        error: false,
...
export const addCommentInPostService = async (req: AuthenticatedRequestBody<AddCommentT>, res: Response, next: NextFunction) => {
  try {
...
export const updateCommentInPostService = async (req: AuthenticatedRequestBody<UpdateCommentT>, res: Response, next: NextFunction) => {
  try {
    ...
    const isAlreadyComment = post.comments.find((item: { user: IUser; _id: string }) => item.user?._id.toString() === req.user?._id.toString() && item?._id.toString() === commentId.toString());
...
export const getCommentInPostService = async (req: AuthenticatedRequestBody<IUser>, res: Response, next: NextFunction) => {
  try {
    ...
    const isCommentExists = post.comments.find((item: { user: IUser; _id: string }) => item?._id.toString() === commentId.toString());

    if (!isCommentExists) {
      return next(new createHttpError.BadRequest());
    }

    post.comments = post.comments.filter((item: { user: IUser; _id: string }) => item.user?._id.toString() === req.user?._id.toString() && item?._id.toString() === commentId.toString());
...
export const getAllCommentInPostService = async (req: AuthenticatedRequestBody<IUser>, res: Response, next: NextFunction) => {
  try {
    const post = await Post.findById(req.params.postId)
      .populate("author", "name  surname  profileImage  bio")
      .populate("likes.user", "name  surname  profileImage bio")
...
export const getUserCommentInPostService = async (req: AuthenticatedRequestBody<IUser>, res: Response, next: NextFunction) => {
  try {
    const post = await Post.findById(req.params.postId)
      .populate("author", "name  surname  profileImage  bio")
      .populate("likes.user", "name  surname  profileImage bio")
    ...
    const isAlreadyComment = post.comments.find((com: { user: IUser }) => com.user?._id.toString() === req.user?._id.toString());

    if (!isAlreadyComment) {
      return next(createHttpError(403, `Auth Failed (Unauthorized)`));
    }

    post.comments = post.comments.filter((com: { user: IUser }) => com.user?._id.toString() === req.user?._id.toString());
...
export const deleteAllCommentInPostService = async (req: AuthenticatedRequestBody<IUser>, res: Response, next: NextFunction) => {
  try {
    const post = await Post.findById(req.params.postId);
...
export const deleteCommentInPostService = async (req: AuthenticatedRequestBody<UpdateCommentT>, res: Response, next: NextFunction) => {
  try {
    ...
    const isAuthorized = post.comments.find(
      (item: { user: IUser; _id: string }) => (req.user?._id.equals(post.author._id) || item.user?._id.toString() === req.user?._id.toString()) && item?._id.toString() === commentId.toString()
    );
...
export const deleteUserCommentInPostService = async (req: AuthenticatedRequestBody<IUser>, res: Response, next: NextFunction) => {
  try {
    const post = await Post.findById(req.params.postId)
      .populate("author", "name  surname  profileImage  bio")
      .populate("likes.user", "name  surname  profileImage bio")
    ...
    const isAlreadyComment = post.comments.find((item: { user: IUser }) => (item.user?._id.toString() === req?.body?.userId ? req?.body?.userId?.toString() : req.user?._id.toString()));
    ...
    post.comments = post.comments.filter((item: { user: IUser }) => item.user?._id.toString() !== (req?.body?.userId ? req?.body?.userId?.toString() : req.user?._id.toString()));
