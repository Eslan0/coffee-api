...
import { AuthenticatedRequestBody, IUser, IOrder, IProcessingOrder } from "../interfaces";
...
export const getOrdersService = async (req: AuthenticatedRequestBody<IUser>, res: Response, next: NextFunction) => {
  try {
    const orders = await Order.find({ "user.userId": req.user?._id })
      .populate("user.userId", "-password -confirmPassword  -status -cart -role -status -isVerified -isDeleted -acceptTerms")
...
export const postOrderService = async (req: AuthenticatedRequestBody<ProcessingOrderT>, res: Response, next: NextFunction) => {
  try {
    ...
    const itemTotalAmount = finalItemsToOrder.reduce((accumulator: number, currentValue: { product: string; quantity: number }) => accumulator + currentValue.quantity, 0);
...
export const clearSingleOrderService = async (req: AuthenticatedRequestBody<IUser>, res: Response, next: NextFunction) => {
  try {
    const { orderId } = req.params;
    const order = await Order.findById(orderId);
...
export const clearAllOrdersService = async (req: AuthenticatedRequestBody<IUser>, res: Response, next: NextFunction) => {
  try {
...
export const getInvoicesService = async (req: AuthenticatedRequestBody<IUser>, res: Response, next: NextFunction) => {
  try {
    ...
    if (order.user?.userId?._id.toString() !== req?.user?._id.toString() && req?.user?.role !== authorizationRoles?.client) {
