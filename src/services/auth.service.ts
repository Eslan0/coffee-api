...
export const verifyEmailService = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!user) return next(createHttpError(400, "Email verification token is invalid or has expired. Please click on resend for verify your Email."));
...
export const getAuthProfileService = async (req: AuthenticatedRequestBody<IUser>, res: Response, next: NextFunction) => {
  try {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const { password: pass, confirmPassword, isVerified, isDeleted, status, acceptTerms, role, ...otherUserInfo } = user._doc;
...
export const updateAuthService = async (req: AuthenticatedRequestBody<IUser>, res: Response, next: NextFunction) => {
  const { name, surname, email, dateOfBirth, gender, mobileNumber, bio, companyName, nationality, address, favoriteAnimal, jobTitle, acceptTerms } = req.body;
