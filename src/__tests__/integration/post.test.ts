...
import { adminEmails, authorizationRoles, postPayload, userPayload, validMongooseObjectId, correctFilePath as localFilePath } from "@src/constants";
  ...
  describe("GET /api/v1/feed/posts/timeline", () => {
    ...
    describe("given the user is logged in and authorized but no post exist in DB ", () => {
      it("should return a 200 status with a json contain empty array", async () => {
        ...
        if (token) {
          try {
            const response = await request(app).get("/api/v1/feed/posts/timeline").set("Authorization", `Bearer ${token}`).send({ postId: validMongooseObjectId, commentId: validMongooseObjectId });
    ...
    describe("given no post in db", () => {
      it("should return a 200 status with a json contain empty array", async () => {
        try {
          ...
          if (token) {
            const response = await request(app).get("/api/v1/feed/posts/user-posts").set("Authorization", `Bearer ${token}`).send({ postId: validMongooseObjectId, commentId: validMongooseObjectId });
    ...
    describe("given the user is logged in and has 3 post in DB", () => {
      it("should return a 200 status with a json contain array of 3 posts", async () => {
        try {
          ...
          if (token) {
            const response = await request(app).get("/api/v1/feed/posts/user-posts").set("Authorization", `Bearer ${token}`);
  ...
  describe("GET /api/v1/feed/posts/:postId", () => {
    ...
    describe("given the post does not exist", () => {
      it("should return a 400 status", async () => {
        try {
          const response = await request(app).get(`/api/v1/feed/posts/${validMongooseObjectId}`).set("Accept", "application/json").expect("Content-Type", /json/);
          expect(response.body).toMatchObject({
            data: null,
            success: false,
            error: true,
    ...
    describe("given the post does exist", () => {
      it("should return a 200 status and the product", async () => {
        try {
          ...
          await post.save();
          const response = await request(app).get(`/api/v1/feed/posts/${post?._id}`).set("Accept", "application/json").expect("Content-Type", /json/);
    ...
    describe("given post id is not valid ", () => {
      it("should return a 422 status with validation message", async () => {
        try {
          ...
          if (token) {
            const response = await request(app).delete("/api/v1/feed/posts/postId").set("Authorization", `Bearer ${token}`);
  ...
  describe("DELETE /api/v1/feed/posts/{postId}", () => {
    ...
    describe("given the user is logged in and authorized and the post does exist", () => {
      it("should delete the post and return a 200 status", async () => {
        ...
        if (token) {
          try {
            const response = await request(app).delete(`/api/v1/feed/posts/${post._id}`).set("Authorization", `Bearer ${token}`);
