...
import { adminEmails, authorizationRoles, cloudinaryResponse, correctFilePath as localFilePath, productPayload, userPayload } from "@src/constants";
  ...
  describe("GET /api/v1/products/:productId", () => {
    ...
    describe("given the product does exist", () => {
      it("should return a 200 status and the product", async () => {
        ...
        const logUser = await request(app).post("/api/v1/auth/login").send({ email: userPayload.email, password: userPayload.password });
        ...
        const finalResult = await request(app).get(`/api/v1/products/${productId}`).set("Accept", "application/json").expect("Content-Type", /json/).expect(200);
  ...
  describe("POST /api/v1/admin/products/add", () => {
    ...
    describe("given the user is logged in and the logged user is not admin", () => {
      it("should return a 403 status with a json message - unauthorized", async () => {
        ...
        // Login to get token and user id
        const authResponse = await request(app).post("/api/v1/auth/login").send({ email: userPayload.email, password: userPayload.password });
